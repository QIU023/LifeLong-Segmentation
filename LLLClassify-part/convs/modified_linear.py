...
class SplitCosineLinear(nn.Module):
    def __init__(self, in_features, out_features1, out_features2, sigma=True):
        ...
        self.out_features = out_features1 + out_features2           #总输出维度
        self.fc1 = CosineLinear(in_features, out_features1, False)  #前k1类维度
        self.fc2 = CosineLinear(in_features, out_features2, False)  #后k2类维度
        if sigma:
            self.sigma = nn.Parameter(torch.Tensor(1))
            self.sigma.data.fill_(1)
        else:
    ...
    def forward(self, x):
        out1 = self.fc1(x)                      #特征向量在前k1类的输出
        out2 = self.fc2(x)                      #特征向量在后k2类的输出
